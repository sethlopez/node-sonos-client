...
var Player = (function (_EventEmitter) {
  ...
  function Player(playerInfo) {
    ...
    this.mediaRenderer = new _devices.MediaRenderer();
    this.mediaServer = new _devices.MediaServer();

    this.services = Object.assign({}, {
      alarmClock: new _services.AlarmClock(),
      deviceProperties: new _services.DeviceProperties(),
      groupManagement: new _services.GroupManagement(),
      musicServices: new _services.MusicServices(),
      systemProperties: new _services.SystemProperties(),
      qplay: new _services.QPlay(),
      zoneGroupTopology: new _services.ZoneGroupTopology()
    });
  }
  ...
  _createClass(Player, [{
    ...
    value: function init() {
      ...
      _upnp2['default'].get(this.description).then(function (response) {
        ...
        _this.emit('error', new _errors.PlayerError(error.message));
      });
    }
    ...
    value: function _doAction(action, data) {
      ...
      return _upnp2['default'].post(this.address, action, data).then(function (response) {
        ...
        _this2.emit('error', new _errors.PlayerError(action + ': ' + error.message));
      });
    }
  }]);
